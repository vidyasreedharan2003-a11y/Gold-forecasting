...
@st.cache_resource
def load_artefacts():
    model         = load_model("gold_bilstm_model.keras")
    scaler        = joblib.load("gold_scaler.pkl")
    target_scaler = joblib.load("gold_target_scaler.pkl")
    features      = pd.read_csv("gold_features.csv")["feature"].tolist()
...
def run_forecast(model, scaler, target_scaler, features):
    ...
    full_scaled = scaler.transform(df_clean.values)
    ...
    result = pd.DataFrame({
        ...
        "Direction"       : ["▲ Up" if r >= 0 else "▼ Down" for r in pred_ret],
    })
...
with st.sidebar:
    ...
    horizon_display = st.slider("Forecast days to display", 1, HORIZON, HORIZON)
...
if st.button("🚀 Run Forecast", type="primary", use_container_width=True):
    ...
    total_change = ((result["Predicted Close"].iloc[-1] - last_price) / last_price * 100)
    up_days      = (result["Return"] >= 0).sum()

    col1.metric("Last Known Close",    f"₹{last_price:,.2f}", str(last_date.date()))
    col2.metric("Day 1 Forecast",      f"₹{result['Predicted Close'].iloc[0]:,.2f}", f"{result['Change %'].iloc[0]:+.2f}%")
    col3.metric(f"Day {horizon_display} Forecast", f"₹{result['Predicted Close'].iloc[horizon_display-1]:,.2f}", f"{total_change:+.2f}% total")
    col4.metric("Up Days",             f"{up_days} / {HORIZON}", "bullish" if up_days >= 6 else "bearish")
    ...
    styled = (display_df.style
              ...
              .format({"Predicted Close": "₹{:,.2f}", "Return": "{:+.4%}", "Change %": "{:+.3f}%"}))
    ...
    if show_chart:
        ...
        ax.plot(recent.index, recent.values, color="#58A6FF", lw=1.5, label="Historical Close")

        future_dates_plot = pd.bdate_range(start=df_raw.index[-1] + pd.Timedelta(days=1), periods=horizon_display)
        fp  = result["Predicted Close"].values[:horizon_display]
        unc = np.array([abs(fp[i]) * (0.008 + i * 0.002) for i in range(len(fp))])

        ax.plot(future_dates_plot, fp, color="#F78166", lw=2.5, ls="--", label=f"{horizon_display}-Day Forecast", zorder=5)
        ax.fill_between(future_dates_plot, fp - unc, fp + unc, color="#F78166", alpha=0.2)
        ax.axvline(df_raw.index[-1], color="#E3B341", ls=":", lw=1.5, alpha=0.8)
        ax.tick_params(colors="#C9D1D9", labelsize=9)
        ax.set_ylabel("Price (₹)", color="#C9D1D9")
        ax.xaxis.set_major_formatter(mdates.DateFormatter("%d %b"))
        ax.xaxis.set_major_locator(mdates.WeekdayLocator(interval=2))
        plt.setp(ax.xaxis.get_majorticklabels(), rotation=25, ha="right", color="#C9D1D9")
    ...
    if show_returns:
        ...
        colors   = ["#3FB950" if v >= 0 else "#F85149" for v in ret_vals]
        ax2.bar(range(1, len(ret_vals) + 1), ret_vals, color=colors, width=0.6, alpha=0.85)
        ax2.axhline(0, color="#6E7681", lw=0.8)
        for i, v in enumerate(ret_vals):
            ax2.text(i + 1, v + np.sign(v) * 0.01, f"{v:+.2f}%", ha="center",
                     va="bottom" if v >= 0 else "top", fontsize=8, color="#C9D1D9")
    ...
    if show_history:
        ...
        hist_df = df_raw[["Close"]].rename(columns={"Close": "Close Price (₹)"})
        st.line_chart(hist_df, color="#58A6FF", use_container_width=True)
